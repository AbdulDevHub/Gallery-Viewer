...
.prependTo('body')
.click(()=>{ var customWindow = window.open(' ', '_self', ''); customWindow.close();  })

var lang = chrome.i18n.getUILanguage();
//open(chrome.runtime.getURL('script/album.html'),'_blank')
$('#gallery').hover(
    (e)=>{$(e.currentTarget).css('opacity','0.6') },
    (e)=>{$(e.currentTarget).css('opacity','1') })
.click(()=>{
  open(chrome.runtime.getURL('script/album.html'),'_self')
})

  function new_search({title:title,url:url,img:img,url2:url2='',size='35'}){
    $(`<div id="item"style="box-shadow: 10px 10px 25px -8px rgba(0,3,20,1);
      ...
      style="display:inline-block;margin:0 auto;height:${size}px;width:${size}px;z-index:99;"
        src="${img}"><div>`)
  .appendTo('#banner')
  .hover( function(){$(this).css('border','3px solid orange')}, function(){$(this).css('border','')}  )
  .click(function(){
    //document.title=$(this).attr('href')
    if($(this).css('opacity')=='1'){$(this).css('opacity',0.2).attr('class','off') }
        else{$(this).css('opacity',1).attr('class','on')}
        })
  ...
  .click((e)=>{
       ...
       $('#name,#url_1').val('Corgi! '+$(e.currentTarget).attr('src').slice(-5,-4))
  ...
  $('.links').hover(
    (e)=>{$(e.currentTarget).css('opacity','1') },
    (e)=>{$(e.currentTarget).css('opacity','0.8') })
